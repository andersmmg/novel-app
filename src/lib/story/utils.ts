...
export function convertDates(obj: any): any {
	...
	for (const [key, value] of Object.entries(obj)) {
		if (
			...
			result[key] = Number.isNaN(date.getTime()) ? new Date() : date
		}
...
export function addFrontmatterIfNeeded(file: {
	...
	if (file.created)
		mergedMetadata.created = file.created
	if (file.edited)
		mergedMetadata.edited = file.edited
	if (file.order !== undefined)
		mergedMetadata.order = file.order
...
export function renameStoryFolder(
	...
	if (!story || !folderPath || !isFolderPath(folderPath))
		return false
